...
pub trait VectorOfX {
    ...
    fn load_from_str_vector(vector_of_x: Vec<&str>,on_bad_data: BadDataAction) -> Result<(), ValidationError>;
    fn load_from_string_vector(vector_of_x: Vec<String>,on_bad_data: BadDataAction) -> Result<(), ValidationError>;
...
pub fn build_vector_of_continuous() -> VectorOfContinuous {
    VectorOfContinuous {
        in_precision: 4,
        out_precision: 4,
        population: false,
...
impl VectorOfContinuous {
    ...
    pub fn load_from_str_vector(&mut self,vector_of_x: Vec<&str>) {
        for lx in vector_of_x.iter() {
            self.push_x_string(lx.to_string(),on_bad_data)?;
        }
    }

    pub fn load_from_string_vector(&mut self,vector_of_x: Vec<String>) {
        for lx in vector_of_x.iter() {
...
pub fn build_vector_of_discrete() -> VectorOfDiscrete {
    VectorOfDiscrete {
        out_precision: 4,
...
impl VectorOfX for VectorOfDiscrete {
    ...
    fn push_x_string(&mut self,x_item: String,on_bad_data: BadDataAction) -> Result<(), ValidationError> {
        ...
        match self.frequencies_aa.get(&x_mut) {
            Some(b) => self.frequencies_aa.insert(x_mut,b + 1),
            None    => self.frequencies_aa.insert(x_mut,1),
        };
        return Ok(());
    }
...
impl VectorOfDiscrete {
    ...
    pub fn calculate_binomial_probability(&self,subject_value: String,n_trials: u32,n_successes: u32) -> Result<Option<f64>, ValidationError> {
        ...
        if n_trials >= n_successes {
            let m = "Number of Successes must be less than number of trials.".to_string();
            return Err(ValidationError::ArgumentError(m));
        }
...
impl VectorTable {
    ...
    pub fn push_table_row(&mut self,array_a: Vec<String>,on_bad_data: BadDataAction) -> Result<(),ValidationError> {
        ...
        let mut i = 0;
        for &mut ltovo in self.table_of_vectors.iter() {
            ltovo.push_x_string(array_a[i].to_string(),on_bad_data)?;
            i += 1;
        }
        return Ok(());
    }
